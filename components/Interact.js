...
import {CheckTotalSupply_Moralis, AllowListMint_Moralis, AuctionMint_Moralis, PublicMint_Moralis, DevMint_Moralis, CheckContractName_Moralis, CheckContractSymbol_Moralis, CheckPreSaleActive_Moralis, CheckAuctionActive_Moralis, CheckPublicSaleActive_Moralis, CheckOwner_Moralis,
  SetPreSaleActive_Moralis, SetAuctionActive_Moralis, SetPublicSaleActive_Moralis, PausePublicSale_Moralis, PausePreSale_Moralis, PauseAuction_Moralis, CheckTokenURI_Moralis, UpdateBaseURI_Moralis, CheckIfRevealed_Moralis, Reveal_Moralis, Withdraw_Moralis, SetPlatformRoyalty_Moralis} from "../JS/local_web3_Moralis";
...
function  Interact() {
  ...
  function HideDisplays(){
    ...
    document.getElementById('Interact_CheckAuctionActiveDisplay').innerText = "";
    document.getElementById('Interact_SetAuctionActiveDisplay').innerText = "";
    document.getElementById('Interact_AuctionDisplay').innerText = "";

    document.getElementById('Interact_DevMintDisplay').innerText = "";
        ...
        <div className={styles.gridContainer_2}>         
          ...
          <input className={styles.interactButton} type="submit" value="Set Presale Active" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && 
            ...
            then(() => {
              ...
              if(error.data && error.data.message){document.getElementById('Interact_SetPreSaleActiveDisplay').innerText = error.data.message;}
              else {document.getElementById('Interact_SetPreSaleActiveDisplay').innerText = error.message;}  
              document.getElementById('Interact_SetPreSaleActiveDisplay').style.visibility = "visible";
              process.exitCode = 1;
            })} >
          </input>
          ...
          <input className={styles.interactButton} type="submit" value="Pause Presale" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && 
            PausePreSale_Moralis((JSON.stringify(selectedValue)).split('"')[5]).
            then(() => {
              document.getElementById('Interact_PausePreSaleDisplay').innerText = "Presale paused!";
              document.getElementById('Interact_PausePreSaleDisplay').style.visibility = "visible";
            }).
            catch((error) => {
              console.error(error);
              console.log("set sale error code: " + error.code);
              console.log("set sale error message: " + error.message);
              if(error.data && error.data.message){document.getElementById('Interact_PausePreSaleDisplay').innerText = error.data.message;}
              else {document.getElementById('Interact_PausePreSaleDisplay').innerText = error.message;}  
              document.getElementById('Interact_PausePreSaleDisplay').style.visibility = "visible";
              process.exitCode = 1;
            })} >
          </input>
          <div className={style.display} id="Interact_PausePreSaleDisplay"></div>

          <div> 
            <input className={styles.interactButtonMint} type="submit" value="Allow List Mint" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && 
              ...
              then(() => {
                ...
                if(error.data && error.data.message){document.getElementById('Interact_AllowListMintDisplay').innerText = error.data.message;}
                else {document.getElementById('Interact_AllowListMintDisplay').innerText = error.message;}  
                document.getElementById('Interact_AllowListMintDisplay').style.visibility = "visible";
                process.exitCode = 1;
              })} >
            </input>
            <input className={styles.gridItemMint} id="Interact_AllowListMintInput" type="number" placeholder='1' min="1" max="100"></input>
          </div> 
          ...
          <input className={styles.interactButton} type="submit" value="Check Auction Active" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && CheckAuctionActive_Moralis((JSON.stringify(selectedValue)).split('"')[5])}></input>
          <div className={style.display} id="Interact_CheckAuctionActiveDisplay"></div>
          
          <input className={styles.interactButton} type="submit" value="Set Auction Active" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && 
            SetAuctionActive_Moralis((JSON.stringify(selectedValue)).split('"')[5]).
            then(() => {
              document.getElementById('Interact_SetAuctionActiveDisplay').innerText = "Auction activated!";
              document.getElementById('Interact_SetAuctionActiveDisplay').style.visibility = "visible";
            }).
            catch((error) => {
              console.error(error);
              console.log("set auction error code: " + error.code);
              console.log("set auction error message: " + error.message);
              if(error.data && error.data.message){document.getElementById('Interact_SetAuctionActiveDisplay').innerText = error.data.message;}
              else {document.getElementById('Interact_SetAuctionActiveDisplay').innerText = error.message;}    
              document.getElementById('Interact_SetAuctionActiveDisplay').style.visibility = "visible";
              process.exitCode = 1;
            })} >
          </input>
          <div className={style.display} id="Interact_SetAuctionActiveDisplay"></div>

          <input className={styles.interactButton} type="submit" value="Pause Auction" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && 
            PauseAuction_Moralis((JSON.stringify(selectedValue)).split('"')[5]).
            then(() => {
              document.getElementById('Interact_PauseAuctionDisplay').innerText = "Auction paused!";
              document.getElementById('Interact_PauseAuctionDisplay').style.visibility = "visible";
            }).
            catch((error) => {
              console.error(error);
              console.log("set auction error code: " + error.code);
              console.log("set auction error message: " + error.message);
              if(error.data && error.data.message){document.getElementById('Interact_PauseAuctionDisplay').innerText = error.data.message;}
              else {document.getElementById('Interact_PauseAuctionDisplay').innerText = error.message;}    
              document.getElementById('Interact_PauseAuctionDisplay').style.visibility = "visible";
              process.exitCode = 1;
            })} >
          </input>
          <div className={style.display} id="Interact_PauseAuctionDisplay"></div>

          <div>
            <input className={styles.interactButtonMint} type="submit" value="Auction Mint" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && 
              AuctionMint_Moralis((JSON.stringify(selectedValue)).split('"')[5]).
              then(() => {
                document.getElementById('Interact_AuctionDisplay').innerText = "Minted!";
                document.getElementById('Interact_AuctionDisplay').style.visibility = "visible";
              }).
              catch((error) => {
                console.error(error);
                console.log("auction error code: " + error.code);
                console.log("auction error message: " + error.message);
                if(error.data && error.data.message){document.getElementById('Interact_AuctionDisplay').innerText = error.data.message;}
                else {document.getElementById('Interact_AuctionDisplay').innerText = error.message;}    
                document.getElementById('Interact_AuctionDisplay').style.visibility = "visible";
                process.exitCode = 1;
              })} >
            </input>
            <input className={styles.gridItemMint} id="Interact_AuctionInput" type="number" placeholder='1' min="1" max="100"></input>
          </div>
          <div className={style.display} id="Interact_AuctionDisplay"></div>

          <div className={style.verticalSpacer}></div>
          <div className={style.verticalSpacer}></div>


          <input className={styles.interactButton} type="submit" value="Check Public Sale Active" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && CheckPublicSaleActive_Moralis((JSON.stringify(selectedValue)).split('"')[5])}></input>
          <div className={style.display} id="Interact_CheckSaleActiveDisplay"></div>
          
          <input className={styles.interactButton} type="submit" value="Set Public Sale Active" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && 
            ...
            then(() => {
              ...
              console.log("set sale error code: " + error.code);
              console.log("set sale error message: " + error.message);
              if(error.data && error.data.message){document.getElementById('Interact_SetSaleActiveDisplay').innerText = error.data.message;}
              else {document.getElementById('Interact_SetSaleActiveDisplay').innerText = error.message;}  
              document.getElementById('Interact_SetSaleActiveDisplay').style.visibility = "visible";
              process.exitCode = 1;
            })} >
          </input>
          ...
          <input className={styles.interactButton} type="submit" value="Pause Public Sale" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && 
            PausePublicSale_Moralis((JSON.stringify(selectedValue)).split('"')[5]).
            then(() => {
              document.getElementById('Interact_PauseSaleDisplay').innerText = "Sale paused!";
              document.getElementById('Interact_PauseSaleDisplay').style.visibility = "visible";
            }).
            catch((error) => {
              console.error(error);
              console.log("set sale error code: " + error.code);
              console.log("set sale error message: " + error.message);
              if(error.data && error.data.message){document.getElementById('Interact_PauseSaleDisplay').innerText = error.data.message;}
              else {document.getElementById('Interact_PauseSaleDisplay').innerText = error.message;}  
              document.getElementById('Interact_PauseSaleDisplay').style.visibility = "visible";
              process.exitCode = 1;
            })} >
          </input>
          <div className={style.display} id="Interact_PauseSaleDisplay"></div>

          <div> 
            <input className={styles.interactButtonMint} type="submit" value="Public Mint" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && 
              ...
              then(() => {
                ...
                console.log("mint error code: " + error.code);
                console.log("mint error message: " + error.message);
                if(error.data && error.data.message){document.getElementById('Interact_MintDisplay').innerText = error.data.message;}
                else {document.getElementById('Interact_MintDisplay').innerText = error.message;}    
                document.getElementById('Interact_MintDisplay').style.visibility = "visible";
                process.exitCode = 1;
              })} >
            </input>
            <input className={styles.gridItemMint} id="Interact_MintInput" type="number" placeholder='1' min="1" max="100"></input>
          </div>
          ...
          <div>  
            <input className={styles.interactButtonMint} type="submit" value="Dev Mint" onClick={() => ValidateSelection((JSON.stringify(selectedValue)).split('"')[5]) && 
              DevMint_Moralis((JSON.stringify(selectedValue)).split('"')[5]).
              then(() => {
                document.getElementById('Interact_DevMintDisplay').innerText = "Minted!";
                document.getElementById('Interact_DevMintDisplay').style.visibility = "visible";
              }).
              catch((error) => {
                console.error(error);
                console.log("dev mint error code: " + error.code);
                console.log("dev mint error message: " + error.message);
                if(error.data && error.data.message){document.getElementById('Interact_DevMintDisplay').innerText = error.data.message;}
                else {document.getElementById('Interact_DevMintDisplay').innerText = error.message;}    
                document.getElementById('Interact_DevMintDisplay').style.visibility = "visible";
                process.exitCode = 1;
              })} >
            </input>   {/*<br></br> */}
            <input className={styles.gridItemMint} id="Interact_DevMintInput" type="number" placeholder='1' min="1" max="100"></input>
          </div>
          <div className={style.display} id="Interact_DevMintDisplay"></div>
