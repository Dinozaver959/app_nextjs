...
contract CONTRACT_NAME is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAX_TO_MINT = 10;
    uint256 public constant MAX_SUPPLY = 10000;
    uint256 public constant MINT_PRICE = 0.095 ether;
    ...
    uint256 private _platformRoyalty = 200; // 1000 = 100%
    ...
    function mint(uint256 numTokens) external payable {
        ...
        require(
            msg.value >= MINT_PRICE.mul(numTokens),
        ...
        payable
    {
        ...
        require(
            _allowListClaimed[msg.sender].add(numTokens) <= MAX_ALLOW_LIST_MINT,
    ...
    function withdraw() public onlyOwnerOrPlatform {
        // used to be only owner
        uint256 balanceUnits = address(this).balance / 1000;
        Address.sendValue(
            payable(_platformAddress),
            _platformRoyalty * balanceUnits
        );
        Address.sendValue(
            payable(msg.sender),
            (1000 - _platformRoyalty) * balanceUnits
        );
    }

    // PLATFORM ONLY

    function setPlatformRoyalties(uint256 royalty_) public onlyPlatform {
        require(
            0 <= royalty_ && royalty_ <= 1000,
            "Platform royalty should be non-negative, up to 1000"
        );

        _platformRoyalty = royalty_;
    }
