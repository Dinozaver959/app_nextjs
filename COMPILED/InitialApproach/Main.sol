...
contract InitialApproach is ERC721, ERC721Enumerable, Ownable {
    using SafeMath for uint256;

    uint256 public constant MAX_PER_ADDRESS_DURING_MINT_ALLOWLIST=2;
    uint256 public constant MAX_SUPPLY=100;
    uint256 public constant MINT_PRICE_PUBLIC=0.095 ether;
    uint256 public constant MINT_PRICE_ALLOWLIST=MINT_PRICE_PUBLIC;
    ...
    constructor() ERC721("Initial Approach", "IA") {}
    ...
    function publicSaleMint(uint256 numTokens) external payable {
        ...
        require(publicSaleActive, "The sale is not active");
        require(numTokens <= MAX_SUPPLY, "Invalid number of tokens");
        require(
            ts.add(numTokens) <= MAX_SUPPLY,
            "Purchase would exceed max tokens"
        );
        require(
            msg.value >= MINT_PRICE_PUBLIC.mul(numTokens),
            "Ether value sent is below the required price"
        );
    ...
    function mintAllowList(uint256 numTokens, bytes32[] calldata proof)
        external
        payable
    {
        require(
        ...
        require(
            _allowListClaimed[msg.sender].add(numTokens) <= MAX_PER_ADDRESS_DURING_MINT_ALLOWLIST,
        ...
        require(
            msg.value >= MINT_PRICE_ALLOWLIST.mul(numTokens),
            "Ether value sent is below the required price"
        );

        _allowListClaimed[msg.sender] = _allowListClaimed[msg.sender].add(
            numTokens
        );
        for (uint256 i = 0; i < numTokens; i++) {
            _safeMint(msg.sender, ts.add(i));
    ...
    /* */
    function numAvailableToMintDuringPreSale(bytes32[] calldata proof)
        external
        view
        returns (uint256)
    {
        if (_verify(_leaf(msg.sender), proof)) {
            return MAX_PER_ADDRESS_DURING_MINT_ALLOWLIST - _allowListClaimed[msg.sender];
        }
        return 0;
    }
    ...
    function setAllowListSaleActive(bool _allowListSaleActive)
        public
        onlyOwner
    {
    ...
    */
    ...
    function withdraw() public onlyOwnerOrPlatform {
        ...
        onlyPlatform
    {
        ...
        PLATFORM_ROYALTY = newRoyaltyNumber;
    }
     ...
     * @dev Returns the address of the current owner.
     */
    function platform() private view returns (address) {
    ...
    modifier onlyOwnerOrPlatform() {
        require(
            owner() == _msgSender() || platform() == _msgSender(),
