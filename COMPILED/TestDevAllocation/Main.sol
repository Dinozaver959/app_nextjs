...
contract TestDevAllocation is Ownable, ERC721A, ReentrancyGuard {
  ...
  uint256 public immutable MAX_PER_ADDRESS_DURING_MINT_PUBLIC=7;
  uint256 public immutable MAX_PER_ADDRESS_DURING_MINT_ALLOWLIST=0;

  // max to mint in total
  uint256 public immutable AMOUNT_FOR_DEVS=18;
  ...
  // CONSTRUCTOR
  constructor() ERC721A("test dev alloc", "tda", MAX_PER_ADDRESS_DURING_MINT_PUBLIC, MAX_SUPPLY){}
  ...
  function devMint(uint256 numTokens) external onlyOwner {
    ...
    uint256 left = numTokens - numTokens / MAX_PER_ADDRESS_DURING_MINT_PUBLIC;
